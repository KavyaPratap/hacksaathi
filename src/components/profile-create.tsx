...
const SKILLS_OPTIONS = [
  ...
  'UI Design', 'UX Design', 'Figma', 'Adobe XD', 'Prototyping',
...
type Step = 'name' | 'photo' | 'location' | 'gender' | 'skills' | 'interests' | 'experience' | 'socials' | 'bio' | 'review' | 'done';
    ...
    startTransition(async () => {
      ...
      if (currentStep === 'name') {
        ...
        if (!textResult.isSafe) {
          toast({ variant: 'destructive', title: "Inappropriate Name", description: textResult.reason });
          return;
        }
      }
      if (currentStep === 'bio') {
         if (!profileData.bio.trim()) {
          toast({ variant: 'destructive', title: "Please write a bio."});
          return;
        }
        const textResult = await proctorText({ text: profileData.bio });
        if (!textResult.isSafe) {
          toast({ variant: 'destructive', title: "Inappropriate Bio", description: textResult.reason });
          return;
        }
      }
      ...
      reader.onloadend = async () => {
        ...
        startTransition(async () => {
          ...
          if (!imageResult.isSafe) {
            toast({ variant: 'destructive', title: "Inappropriate Image", description: imageResult.reason });
            return;
          }
  ...
  const handleGenerateBio = () => {
    startTransition(async () => {
      try {
        ...
        toast({ title: "Bio Generated!", description: "Feel free to edit it as you like."})
      } catch (error) {
        toast({ variant: 'destructive', title: "Error generating bio", description: "Please try again."});
      }
    });
  };
  ...
  const handleProfileSubmit = async () => {
    ...
    startTransition(async () => {
        try {
            ...
            toast({ title: "Profile Created!", description: "Welcome to HackSaathi!"});
        ...
        navigator.geolocation.getCurrentPosition(async (position) => {
            ...
            try {
                ...
                if (city && country) {
                    ...
                    toast({ title: "Location Detected!", description: locationString });
                } else {
            ...
            toast({ variant: 'destructive', title: "Unable to retrieve your location. Please grant permission." });
        });
    });
  };
  ...
  const renderStepContent = () => {
    switch(currentStep) {
        ...
        return (
          <Message isUser={false}>
            ...
            <div className='mt-4 flex gap-2'>
              ...
              <Button onClick={() => handleNextStep('gender', <blockquote>{profileData.location}</blockquote>)} disabled={isPending || !profileData.location}>Next</Button>
            </div>
          </Message>
        );
      case 'gender':
      ...
      case 'skills':
          return (
            <Message isUser={false}>
                 <h3 className="font-semibold mb-2">What are your top skills or technologies you work with? Select a few.</h3>
                 ...
                 <div className='mt-4 flex gap-2'>
                    ...
                    <Button onClick={() => handleNextStep('interests', <blockquote>{profileData.techStack.join(', ')}</blockquote>)}>Next Step</Button>
                 </div>
            </Message>
          )
      case 'interests':
