...
export const Chat = () => {
  ...
  return (
    <div className="grid h-[calc(100vh-var(--header-height,0px))] md:grid-cols-[350px_1fr]">
...
const ActiveChatView = ({ conversationId, currentUser, onBack }: { conversationId: string, currentUser: any, onBack: () => void }) => {
    ...
    useEffect(() => {
        const channel = supabase
          ...
          async (payload) => {
              // Avoid adding our own sent messages twice (optimistic vs. realtime)
    ...
    const handleSendMessage = async () => {
        ...
        // Keep only the last 4 timestamps
        while (messageTimestamps.current.length > 4) {
            messageTimestamps.current.shift();
        }

        // Check if 4 messages were sent within 5 seconds
        if (messageTimestamps.current.length === 4 && (now - messageTimestamps.current[0] < 5000)) {
            setIsBlocked(true);
            toast.error("You're messaging too fast!", { description: "Please wait 30 seconds before sending another message."});
            setTimeout(() => {
                setIsBlocked(false);
                messageTimestamps.current = []; // Reset timestamps after block
             ...
             <footer className="border-t p-4 flex-shrink-0 bg-background">
                <div className="relative">
                    ...
                    <Button 
                        ...
                        disabled={isSending || !newMessage.trim() || isBlocked}
                    >
